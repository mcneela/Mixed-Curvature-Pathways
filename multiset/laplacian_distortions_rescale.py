...
def main(dataset):
    ...
    #     "reactome" : 1.0,
    # }
    consts = pickle.load(open(f"{dataset}_laplacian_models.pkl", "rb"))
    ...
    output_csv_file = f"{dataset}_laplacian_rescaled_distortions.csv"

    with open(output_csv_file, 'w', newline='') as csvfile:
        csvwriter = csv.writer(csvfile)
        csvwriter.writerow(["Graph Number", "Distortion"])

        with open(f"{dataset}_laplacian_distortions.csv", "r") as infile:
            reader = csv.reader(infile)
            for i, line in enumerate(reader):
                if i == 0:
                    continue
                graph_num = line[0]
                filename = f"{graph_num}.npy"
                ...
                embedded_nodes = np.load(embedding_file_path)
