...
if __name__ == "__main__":
    ...
    graph_nums, dists, embeds, constants = [], [], [], []
    with open(args.file, 'r') as csvfile:
        ...
        for i, line in enumerate(reader):
            ...
            if "n2v" in args.file:
                ...
                embeddings = np.load(f"{dset}_laplacian_models/{line[0]}.npy")
    ...
    for k in range(len(constants)):
        ...
        for i in range(100):
            optimizer.zero_grad()
            loss = calculate_distortion(graphs[k], k, embeds[k], model.c, dset)
            loss.backward()
            # print(f"Epoch: {i}, Loss: {loss}, C: {model.c.detach().numpy()}")
            optimizer.step()
        print(f"Graph num: {k}, C: {model.c.detach().numpy()}")
        constants[k] = model
        
    import pickle
    if "n2v" in args.file:
        with open(f'{dset}_models.pkl', 'wb') as f:
            pickle.dump(constants, f)
    else:
        with open(f'{dset}_laplacian_models.pkl', 'wb') as f:
            pickle.dump(constants, f)