...
df = pd.read_csv('humancyc/humancyc_all_data.tsv', sep='\t')
...
best_spaces = {}
best_space_counts = {}
with open('humancyc/humancyc_unique_best_spaces.tsv', 'w', newline='') as fpath:
    writer = csv.writer(fpath, delimiter='\t')
    writer.writerow(['Graph num', 'LR', 'Num Nodes', 'Num Edges', 'Density', 'Number of Self Loops', 'Node Connectivity', 'Avg Clustering Coeff', 'Diameter', 'Degree Assortativity', 'Is Bipartite', 'H dim', 'H copies', 'E dim', 'E copies', 'S dim', 'S copies', 'Num Triangles', 'Dist Range', 'Dist Diff from Mean', 'Dist Diff from Next Best', 'Dist Diff from Euclidean', '% Dist Diff From Euclidean', 'Best Euclidean Distortion', 'Best Overall Distortion'])
    for i, graph_num in enumerate(df['Graph num'].unique()):
        graph = df[df['Graph num'] == graph_num]
        bestidx = graph['Best dist'].idxmin()
        lr = df.iloc[bestidx]['LR']
        ...
        try:
            ...
            percent_dist_diff_from_euclidean = 0
        ...
        graph_file = open(f'humancyc/{graph_num}/{graph_num}.edges', 'r')
...
with open('humancyc/general_stats.txt', 'w') as fpath:
