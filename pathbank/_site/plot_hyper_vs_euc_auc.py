...
hyp_aucs = list(map(lambda x: float(x.split('\t')[1]), open('../../hgcn/initial_lp_results_new.tsv').readlines()[1:]))
euc_aucs = list(map(lambda x: float(x.split('\t')[2]), open('../../hgcn/initial_lp_results_new.tsv').readlines()[1:])) 
...
plt.savefig('plots/scatterplot_hyp_vs_euc_auc_new.png')
plt.show()